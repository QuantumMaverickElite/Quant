...
def validate_signal_coverage(args: argparse.Namespace) -> None:
    ...
    if rows == 0 or unique_dates < int(args.min_signal_dates):
        raise SystemExit(
            f"Insufficient base strategy signal coverage for {args.start}..{args.end}: "
            f"rows={rows:,}, dates={unique_dates:,}, required_dates={int(args.min_signal_dates):,}. "
            "For a smoke test, lower --min-signal-dates. For research, rebuild or expand the existing allocator signals."
        )
