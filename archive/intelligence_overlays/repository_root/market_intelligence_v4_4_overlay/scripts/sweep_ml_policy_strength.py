...
def adjusted_confidence(base: pd.Series, ml: pd.Series, *, strength: float, max_abs_delta: float | None) -> pd.Series:
...
def write_sweep_plots(summary: pd.DataFrame, out_dir: Path, *, focus_return_col: str, focus_top_n: int) -> None:
    ...
    labels = [
        f"{r.return_col} top{int(r.top_n)} s{r.strength:g} cap{r.max_abs_delta}"
...
def main() -> None:
    ...
    for strength in args.strengths:
        for cap in caps:
            ...
            df[adj_col] = adjusted_confidence(df[base_col], df[ml_col], strength=float(strength), max_abs_delta=cap)
