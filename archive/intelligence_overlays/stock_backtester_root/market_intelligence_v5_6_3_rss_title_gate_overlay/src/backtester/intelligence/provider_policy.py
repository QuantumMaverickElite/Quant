...
from .entity_resolver import aliases_for_query, query_relevance as resolver_query_relevance
...
PROVIDER_ALIASES = {
    ...
    "yahoo_rss": "rss_yahoo_finance",
    "yahoo_finance_rss": "rss_yahoo_finance",
    "rss_google": "rss_google_news",
    "google_rss": "rss_google_news",
    "google_news_rss": "rss_google_news",
    "gdelt": "gdelt_doc",
...
PROVIDER_POLICIES: dict[str, ProviderPolicy] = {
    ...
    "rss_yahoo_finance": ProviderPolicy(
        provider="rss_yahoo_finance",
        quality_tier="medium",
        min_relevance_score=0.45,
        min_request_interval_seconds=1.0,
        allowed_for_ml_training=False,
        notes="Free Yahoo Finance RSS headline feed. Store/backtest/live-score by default; enable ML only after source review.",
    ),
    "rss_google_news": ProviderPolicy(
        ...
        notes="Google News RSS search results. Discovery-only by default because aggregation/search can be noisy.",
    ),
...
def normalized_query_aliases(query: str) -> list[str]:
    return aliases_for_query(query)


def query_relevance(query: str, text: str) -> tuple[list[str], float]:
    return resolver_query_relevance(query, text)
