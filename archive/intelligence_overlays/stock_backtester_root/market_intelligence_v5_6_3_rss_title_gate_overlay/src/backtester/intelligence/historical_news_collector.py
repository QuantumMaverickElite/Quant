...
def title_supported_search_relevance(
    query: str,
    *,
    title: str,
    description: str,
    url: object = None,
    require_title_entity: bool = False,
    min_title_relevance: float = 0.45,
) -> tuple[list[str], float, dict]:
    """Score search/RSS rows with an optional headline evidence gate.

    RSS feeds can be ticker-routed even when the headline is about an adjacent
    company or a broad market story.  For those feeds, a body/description match
    is useful audit evidence, but by default it should not rescue a headline
    that never names the queried security.
    """
    title_text = str(title or "")
    description_text = str(description or "")
    full_text = f"{title_text}. {description_text}".strip()
    matched, relevance, audit = search_source_relevance(query, text=full_text, url=url)
    title_matched, title_score = text_entity_relevance(query, title_text)
    description_matched, description_score = text_entity_relevance(query, description_text)
    title_score_f = round(float(title_score), 4)
    description_score_f = round(float(description_score), 4)
    title_gate_pass = (not require_title_entity) or title_score_f >= float(min_title_relevance)
    audit.update(
        {
            "title_matched_terms": title_matched,
            "title_relevance_score": title_score_f,
            "description_matched_terms": description_matched,
            "description_relevance_score": description_score_f,
            "requires_title_entity": bool(require_title_entity),
            "title_entity_gate_pass": bool(title_gate_pass),
            "min_title_relevance": float(min_title_relevance),
        }
    )
    if not title_gate_pass:
        audit["pre_title_gate_relevance_score"] = round(float(relevance), 4)
        return title_matched, 0.0, audit
    return matched, relevance, audit
...
def rss_record(
    ...
    *,
    ...
    require_title_entity: bool = True,
) -> HistoricalSourceRecord:
    ...
    matched, relevance, relevance_audit = title_supported_search_relevance(
        query,
        title=title,
        description=description,
        url=url,
        require_title_entity=require_title_entity,
    )
    raw = dict(item)
