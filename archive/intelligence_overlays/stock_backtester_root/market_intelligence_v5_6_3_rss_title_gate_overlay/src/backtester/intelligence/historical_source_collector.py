...
def normalized_aliases(query: str) -> list[str]:
    aliases = query_terms(query)
    if query.upper() not in aliases:
        aliases.append(query.upper())
    out: list[str] = []
    seen: set[str] = set()
...
def relevance_text(article: dict) -> tuple[str, str, str]:
...
def article_matched_terms(query: str, article: dict) -> list[str]:
    title, url, domain = relevance_text(article)
    haystack = f"{title} {url} {domain}".lower()
    return [alias for alias in normalized_aliases(query) if alias in haystack]


def article_relevance_score(query: str, article: dict) -> float:
    title, url, domain = relevance_text(article)
    title_low = title.lower()
    url_low = url.lower()
    domain_low = domain.lower()
    score = 0.0
    aliases = normalized_aliases(query)
    for alias in aliases:
        compact = alias.replace(" ", "").replace("-", "")
        url_compact = url_low.replace("-", "").replace("_", "")
        if alias in title_low:
            score += 0.70
        if alias in url_low or compact in url_compact:
            score += 0.25
        if alias in domain_low:
            score += 0.05
...
def passes_relevance_filter(
    ...
    *,
    ...
    if query.upper() in {"MARKET", "MACRO"}:
        return True
    title, _, _ = relevance_text(article)
    aliases = normalized_aliases(query)
    if strict_title_match and not any(alias in title.lower() for alias in aliases):
        return False
    return article_relevance_score(query, article) >= min_relevance_score
...
def record_from_gdelt_article(query: str, article: dict) -> HistoricalSourceRecord:
    ...
    return HistoricalSourceRecord(
        ...
        matched_terms=article_matched_terms(query, article),
        relevance_score=article_relevance_score(query, article),
        raw=article,
    )
    ...
    *,
    ...
    for query in queries:
        for win_start, win_end in date_windows(start, end, window_days=window_days):
            ...
            print(f"GDELT kept {len(filtered)}/{len(articles)} records after relevance filter")
            records.extend(record_from_gdelt_article(query, article) for article in filtered)
