...
def grounded_event_type(sentence: str, context: str, classified_event_type: str) -> str:
    ...
    if any_term_matches(context_low, financial_metric_terms):
        if any_term_matches(context_low, forward_terms):
            return "guidance"
        if any_term_matches(context_low, ["revenue", "eps", "earnings", "profit", "margin", "rule of 40", "year over year", "commercial"]):
            return "earnings"
