...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-fetches", type=int, help="Maximum provider/query fetch attempts for this invocation.")
...
def main() -> None:
    ...
    if args.expand_entity_search:
        ...
        print(f"Estimated search-style provider requests per provider: up to {estimated}")
        for query in queries[:20]:
            terms = entity_search_terms(query, max_terms=args.max_search_aliases)
            print(f"entity_search_terms {query}: {terms}")
