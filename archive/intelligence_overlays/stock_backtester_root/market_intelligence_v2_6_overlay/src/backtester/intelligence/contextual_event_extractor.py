...
EVENT_TERMS = {
    ...
    "geopolitical": ["war", "sanction", "tariff", "election", "geopolitical", "white house", "congress"],
...
def event_type(text: str) -> str:
    ...
    for candidate, terms in EVENT_TERMS.items():
        count = sum(1 for term in terms if term in low)
        if count > best_count:
            best = candidate
            best_count = count
    return best
...
def horizon(text: str) -> EventHorizon:
    low = text.lower()
    if any(term in low for term in ["today", "intraday", "this morning", "afternoon"]):
        return "intraday"
    if any(term in low for term in ["this week", "next week", "near term", "short term"]):
        return "days"
    if any(term in low for term in ["weeks", "coming weeks", "month", "months"]):
        return "weeks"
    if any(term in low for term in ["quarter", "year", "2026", "2027", "full-year"]):
        return "quarters"
    return "unknown"
...
def magnitude(text: str, sentiment_confidence: float, scope: EventScope) -> float:
    ...
    if any(term in low for term in strong_terms):
        intensity += 0.35
    if any(term in low for term in medium_terms):
        intensity += 0.20
...
def extract_contextual_events(
    ...
    *,
    ...
    for doc in docs:
        ...
        for (sentence, scope), sentiment in zip(candidates, sentiments):
            etype = event_type(sentence)
            events.append(
                MarketEvent(
                    ...
                    magnitude=round(magnitude(sentence, sentiment.confidence, scope), 4),
