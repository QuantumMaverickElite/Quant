...
def bootstrap_summary(
    ...
    *,
    ...
    idx = rng.integers(0, n_dates, size=(iterations, n_dates))
    b = np.nanmean(baseline[idx], axis=1)
    h = np.nanmean(heuristic[idx], axis=1)
    m = np.nanmean(ml[idx], axis=1)
    sims = pd.DataFrame(
        {
    ...
    if baseline_dd is not None and heuristic_dd is not None and ml_dd is not None:
        bdd = np.nanmean(baseline_dd.to_numpy(dtype=float)[idx], axis=1)
        hdd = np.nanmean(heuristic_dd.to_numpy(dtype=float)[idx], axis=1)
        mdd = np.nanmean(ml_dd.to_numpy(dtype=float)[idx], axis=1)
        sims["baseline_drawdown"] = bdd
        sims["heuristic_nlp_drawdown"] = hdd
        sims["walk_forward_ml_drawdown"] = mdd
        sims["ml_drawdown_minus_baseline"] = mdd - bdd
        sims["ml_drawdown_minus_heuristic"] = mdd - hdd
    ...
    summary = {
        ...
        "prob_ml_ties_baseline": float(np.isclose(lift, 0.0, atol=1e-12).mean()),
        "prob_ml_ties_heuristic": float(np.isclose(heuristic_lift, 0.0, atol=1e-12).mean()),
        "prob_ml_nonworse_baseline": float((lift >= -1e-12).mean()),
        "prob_ml_nonworse_heuristic": float((heuristic_lift >= -1e-12).mean()),
