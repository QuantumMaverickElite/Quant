...
def term_pattern(term: str, *, ignore_case: bool = True) -> re.Pattern[str]:
    raw_text = str(term or "").strip()
    raw = raw_text.casefold() if ignore_case else raw_text
    flags = re.IGNORECASE if ignore_case else 0
    if raw.startswith("$"):
        return re.compile(r"(?<![A-Za-z0-9])" + re.escape(raw) + r"(?![A-Za-z0-9])", flags)
    return re.compile(r"(?<![A-Za-z0-9])" + re.escape(raw) + r"(?![A-Za-z0-9])", flags)
...
def is_short_ticker_like(term: str) -> bool:
    clean = str(term or "").strip()
    return bool(re.fullmatch(r"[A-Z]{1,5}", clean))


def is_generic_alias(term: str) -> bool:
    words = normalize_name(term).split()
    if not words:
        return True
    if len(words) == 1 and words[0] in GENERIC_ENTITY_WORDS:
        return True
    non_generic = [word for word in words if word not in GENERIC_ENTITY_WORDS]
    return len(non_generic) == 0


def alias_matches_text(term: str, text: str) -> bool:
    clean = str(term or "").strip()
    if not clean:
        return False
    if clean.startswith("$"):
        return bool(term_pattern(clean, ignore_case=True).search(text))
    if is_short_ticker_like(clean):
        if len(clean) == 1:
            return False
        if clean.casefold() in COMMON_SHORT_WORDS:
            return False
        return bool(term_pattern(clean, ignore_case=False).search(text))
    if is_generic_alias(clean):
        return False
    return bool(term_pattern(clean, ignore_case=True).search(text))


def strong_entity_evidence(matched_terms: list[str]) -> bool:
    ...
    for term in matched_terms:
        ...
        if clean.startswith("$"):
            return True
        if is_short_ticker_like(clean) and len(clean) >= 2:
            return True
        words = normalize_name(clean).split()
        if len(words) >= 2 and len(clean) >= 8 and not is_generic_alias(clean):
            return True
...
class EntityResolver:
    ...
    def query_relevance(self, query: str, text: str) -> tuple[list[str], float]:
        ...
        for alias in sorted(aliases, key=len, reverse=True):
            if not alias:
                continue
            if alias_matches_text(alias, body):
    ...
    def resolve_text_to_entities(self, text: str, *, min_score: float = 0.35) -> list[EntityMatch]:
        ...
        for ticker, record in sorted(self.by_ticker.items()):
            ...
            if score >= min_score and strong_entity_evidence(matched):
                matches.append(
                    EntityMatch(
                        ticker=ticker,
