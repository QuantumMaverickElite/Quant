...
def load_summary(port: int) -> pd.DataFrame:
    path = BASE / f"fast_v2_drift_weekly_sample24_port{port}" / "threshold_summary.csv"
...
def main() -> None:
    out_dir = Path("outputs/research/threshold_rebalance_fast_v2_drift")
    out_dir.mkdir(parents=True, exist_ok=True)

    comparison = pd.concat([load_summary(port) for port in PORTS], ignore_index=True)
    comparison = comparison.sort_values(["portfolio_size", "threshold"]).reset_index(drop=True)

    csv_path = out_dir / "fast_v2_drift_threshold_portfolio_comparison.csv"
    comparison.to_csv(csv_path, index=False)

    print("\nFast v2 Drift Threshold Portfolio Comparison")
    print("=" * 150)
    print(
        comparison[
            [
    ...
    best_return_path = out_dir / "best_fast_v2_drift_threshold_by_return.csv"
    best_sharpe_path = out_dir / "best_fast_v2_drift_threshold_by_sharpe.csv"
    ...
    charts = [
        ...
        ("median_sharpe", "Median Sharpe", "median_sharpe_by_threshold_portfolio.png"),
        ...
        ("mean_turnover_pct", "Mean Turnover Per Rebalance (%)", "mean_turnover_by_threshold_portfolio.png"),
    ]
