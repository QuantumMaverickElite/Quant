...
def source_sentiment(row: pd.Series) -> float:
    ...
    try:
        ...
        pass
    try:
        return float(row["model_sentiment_score"])
    except (KeyError, TypeError, ValueError):
        pass
...
def add_policy_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    source_kind = df["source_kind"].astype(str).str.lower()
    df["is_official_filing"] = source_kind.eq("filing") | df["provider_is_official_source"]
    df["is_discovery_news"] = source_kind.str.contains("discovery", na=False) | df["provider_requires_confirmation"]
    df["is_media_news"] = source_kind.isin({"news", "news_sentiment"})
    df["is_market_news"] = df["is_media_news"] & ~df["is_discovery_news"] & ~df["is_official_filing"]
    return df
    ...
    *,
    ...
    for _, signal in signals.iterrows():
        ...
        for window in windows:
            ...
            media_news = news[news["is_media_news"]]
            market_news = news[news["is_market_news"]]
            official_filing = news[news["is_official_filing"]]
            discovery_news = news[news["is_discovery_news"]]
            ...
            row[f"media_news_count_{window}d"] = float(len(media_news))
            row[f"media_news_relevance_sum_{window}d"] = (
                float(media_news["relevance_score"].sum()) if len(media_news) else 0.0
            )
            row[f"media_news_sentiment_quality_weighted_{window}d"] = weighted_mean(
                media_news["sentiment_score"], media_news["policy_relevance_weight"]
            )

            row[f"market_news_count_{window}d"] = float(len(market_news))
            row[f"market_news_relevance_sum_{window}d"] = (
                float(market_news["relevance_score"].sum()) if len(market_news) else 0.0
            )
            row[f"market_news_sentiment_quality_weighted_{window}d"] = weighted_mean(
                market_news["sentiment_score"], market_news["policy_relevance_weight"]
            )

            row[f"official_filing_count_{window}d"] = float(len(official_filing))
            row[f"official_filing_relevance_sum_{window}d"] = (
                float(official_filing["relevance_score"].sum()) if len(official_filing) else 0.0
            )

            row[f"discovery_news_count_{window}d"] = float(len(discovery_news))
            row[f"discovery_news_relevance_sum_{window}d"] = (
                float(discovery_news["relevance_score"].sum()) if len(discovery_news) else 0.0
            )
            row[f"discovery_news_sentiment_quality_weighted_{window}d"] = weighted_mean(
                discovery_news["sentiment_score"], discovery_news["policy_relevance_weight"]
            )
