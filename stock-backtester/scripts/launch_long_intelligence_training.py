...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--download-prices", action=argparse.BooleanOptionalAction, default=True)
...
def profile_args(profile: str) -> dict[str, list[str]]:
    if profile == "compatible_10d":
        return {
            "horizons": ["5", "10", "20"],
            "success_horizon": ["10"],
            "return_cols": ["next_5d_return", "next_10d_return"],
...
def write_run_script(args: argparse.Namespace, *, work_dir: Path, news_sources: list[Path]) -> Path:
    ...
    if args.download_prices:
        train_cmd.append("--download-prices")
    if args.keep_going:
        train_cmd.append("--keep-going")
