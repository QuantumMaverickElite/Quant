...
def run_period(label: str, start: str, end: str, args: argparse.Namespace, manifest_rows: list[dict]) -> None:
    ...
    if args.fetch_sec:
        cmd.append("--fetch-sec")
        if args.sec_user_agent:
            cmd.extend(["--sec-user-agent", args.sec_user_agent])
        cmd.extend(["--sec-forms", *args.sec_forms])
        cmd.extend(["--sec-sleep-seconds", str(args.sec_sleep_seconds)])
    if args.fetch_news:
        cmd.append("--fetch-news")
    fetch_providers = list(args.fetch_providers)
    if args.include_alpha_vantage and "alpha_vantage" not in fetch_providers:
        fetch_providers.append("alpha_vantage")
    if fetch_providers:
        cmd.extend(["--fetch-providers", *fetch_providers])
