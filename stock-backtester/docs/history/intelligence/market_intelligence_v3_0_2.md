# Market Intelligence v3.0.2

Adds the first historical point-in-time source collector.

v3.0.1 added GDELT retry/backoff handling so HTTP 429 rate limits do not crash the whole run.

v3.0.2 adds relevance filtering:

- quoted phrase query support
- `matched_terms`
- `relevance_score`
- `--min-relevance-score`
- `--strict-title-match`
...
python -m scripts.fetch_historical_intelligence_sources \
  ...
  --max-retries 5 \
  --backoff-seconds 10 \
  --min-relevance-score 0.50 \
  --strict-title-match \
  --out data/intelligence/historical/raw/gdelt_pltr_qqq_market_2026_05.jsonl
```

For ticker/company-specific training data, prefer strict filtering first.

For broad macro/market discovery, do not use `--strict-title-match`.
