# Phase 26 output taxonomy and preservation

Phase 26 inventories generated artifacts as of 2026-09-01. It changes no
quantitative code, schema, baseline authority, or generated file. The
...
filesystem contracts. Twenty-five contain durable contract or evidence value,
although durable does not mean canonical.
...
| Worker-derived tables | Chromebook parsers | event fact and outcome builders, legacy heuristics | Keep as external operational interface |
...
| Research reports | backtest, scorecard, comparison, threshold, regime, and Monte Carlo commands | humans and subsequent comparison commands | Keep; promotion requires explicit provenance |

`strategy_scorecard.py` confirms that `outputs/regime/`, `outputs/dividend/`,
and caller-selected output roots are report inputs. It is not evidence that
all similarly named trees are equivalent or canonical.

## Authority-sensitive families

### Signals and correlation

`outputs/signals/` is a high-risk pipeline contract: 52 Parquet files are read
by portfolio, Rust, allocator, intelligence, and evaluation commands. The
inventory does not choose H20 versus H100 authority.
...
No generation is promoted and no paired-curve tree is deleted merely because
an older cleanup proposal called it a candidate.

### Intelligence

The 498,789,670-byte intelligence family is not one authority:

| Subfamily | Bytes | Files | Status |
| --- | ---: | ---: | --- |
| `training_runs/` | 468,305,214 | 1,464 | Historical ML-policy/training evidence with partial manifests |
...
| allocator and strategy comparison tables | about 5.4 MiB | mixed | Research/evaluation evidence, not allocator authority |

Historical ML-policy output is neither current event-learning authority nor
allocator authority. Operational heuristic summaries remain fallback evidence.
The current event-learning tables form a staged reader/writer chain and remain
in place.
...
directory. CSV evidence, plots, and human-readable result notes preserve four
...
H20/H100, threshold-generation, and intelligence-training authority remain
explicitly unresolved. Phase 26 does not convert retained artifacts into
official baselines.
...
This is an evidence-based outcome, not a claim that every retained byte is
permanently valuable.

## Future topology and retention
...
[`output_policy.md`](../output_policy.md). A future retention pass may prune
individual runs only after promotion metadata and replacement hashes exist;
it should not introduce another taxonomy migration.

## Residual physical clutter

| Residual area | Classification |
| --- | --- |
| `signals/`, `correlation/`, `context/`, `cache/`, `rust_inputs/`, `worker_ingest/` | `NECESSARY_PATH_CONTRACT` |
| threshold generations, regime history, dividend history, selected reports | `DURABLE_HISTORY` |
| intelligence training runs and Rust stress trials | `EXPLICITLY_DEFERRED` pending run-level promotion/retention decisions |
| old plots, curve-heavy Monte Carlo, paired threshold curves | `LOW_VALUE_BUT_NOT_WORTH_RISK` without manifests proving replacement |
| recurring training, model, report, and cache growth | `FUTURE_RETENTION_POLICY_WORK` governed by current policy |

No major family remains `UNKNOWN`; uncertainty is recorded at artifact/run
level through provenance and regenerability classifications.

## Freeze readiness

Output organization is sufficient for Reorg V1 freeze. Current and durable
families are discoverable through the inventory, major filesystem contracts
are explicit, caches/scratch have no authority, and new significant runs have
a defined metadata and placement standard. Remaining ugliness is classified
and preserved rather than mistaken for unowned clutter. Another physical
output-reorganization phase is not warranted before freeze.
